...
def notion_time(TIME_ZONE,TIME_ZONE_OFFSET):
    time_now = datetime.now() + timedelta(hours=TIME_ZONE_OFFSET)
    time_param = f'%Y-%m-%dT%H:%M:%S{TIME_ZONE:+03}:00'
    time_str = time_now.strftime(time_param)
    print(time_str)
    return time_str #Change the last 5 characters to be representative of your timezone
    #^^ has to be adjusted for when daylight savings is different if your area observes it
    
def DateTimeIntoNotionFormat(dateTimeValue,TIME_ZONE):
    return dateTimeValue.strftime(f'%Y-%m-%dT%H:%M:%S{TIME_ZONE:+03}:00')  #Change the last 5 characters to be representative of your timezone
    #^^ has to be adjusted for when daylight savings is different if your area observes it

def googleQuery(TIME_ZONE,TIME_ZONE_OFFSET):
    time_now = datetime.now() + timedelta(hours=TIME_ZONE_OFFSET)
    time_param = f'%Y-%m-%dT%H:%M:%S{TIME_ZONE:+03}:00'
    return time_now.strftime(time_param) #Change the last 5 characters to be representative of your timezone
    #^^ has to be adjusted for when daylight savings is different if your area observes it
...
def main(
    ...
    credentialsLocation = "./data/token.pkl"
                ...
                **{
                    ...
                    "properties": {
                        ...
                        LastUpdatedTime_Notion_Name: {
                            "date":{
                                'start': notion_time(TIME_ZONE,TIME_ZONE_OFFSET),
                                'end': None,
                            }
                        }
                    },
            ...
            my_page = notion.pages.update( ##### This updates the last time that the page in Notion was updated by the code
                **{
                    ...
                    "properties": {
                        LastUpdatedTime_Notion_Name: {
                            "date":{
                                'start': notion_time(TIME_ZONE,TIME_ZONE_OFFSET), #has to be adjusted for when daylight savings is different
                                'end': None,
                            }
                        }
                    },
                ...
                my_page = notion.pages.update( #update the notion dashboard with the new datetime and update the last updated time
                    **{
                        ...
                        "properties": {
                            ...
                            LastUpdatedTime_Notion_Name: {
                                "date":{
                                    'start': notion_time(TIME_ZONE,TIME_ZONE_OFFSET), #has to be adjsuted for when daylight savings is different
                                    'end': None,
                                }
                            }
                        },
                ...
                my_page = notion.pages.update( #update the notion dashboard with the new datetime and update the last updated time
                    **{
                        ...
                        "properties": {
                            Date_Notion_Name: {
                                "date":{
                                    'start': DateTimeIntoNotionFormat(start,TIME_ZONE),
                                    'end': DateTimeIntoNotionFormat(end,TIME_ZONE),
                                }
                            },
                            LastUpdatedTime_Notion_Name: {
                                "date":{
    ...
    events = service.events().list(calendarId = calendarID, maxResults = 2000, timeMin = googleQuery(TIME_ZONE,TIME_ZONE_OFFSET) ).execute()
                    ...
                    **{
                        ...
                        "properties": {
                            ...
                            Date_Notion_Name: {
                                "type": 'date',
                                'date': {
                                    'start': DateTimeIntoNotionFormat(calStartDates[i],TIME_ZONE),
                                    'end': DateTimeIntoNotionFormat(calEndDates[i],TIME_ZONE), 
                                }
                            },
                            LastUpdatedTime_Notion_Name: {
                                "type": 'date',
